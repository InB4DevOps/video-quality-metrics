...
def create_table_plot_metrics(json_file_path, args, decimal_places,
    ...
    if args.disable_quality_metrics:
        return

    # VMAF
    vmaf_scores = [frame['metrics']['vmaf'] for frame in file_contents[
        'frames']]
    mean_vmaf = force_decimal_places(round(np.mean(vmaf_scores),
                                           decimal_places), decimal_places)
    min_vmaf = force_decimal_places(round(min(vmaf_scores), decimal_places),
                                    decimal_places)
    # Standard deviation.
    vmaf_std = force_decimal_places(round(np.std(vmaf_scores),
                                          decimal_places), decimal_places)
    vmaf = f'{min_vmaf} | {vmaf_std} | {mean_vmaf}'
    # Plot a line showing the variation of the VMAF score throughout the
    # video.
    ...
    if args.calculate_ssim:
        ssim_scores = [ssim['metrics']['ssim'] for ssim
                       in file_contents['frames']]
        mean_ssim = force_decimal_places(round(np.mean(ssim_scores),
                                               decimal_places),
                                         decimal_places)
        min_ssim = force_decimal_places(round(min(ssim_scores),
                                              decimal_places),
                                        decimal_places)
        # Standard deviation.
        ssim_std = force_decimal_places(round(np.std(ssim_scores),
                                              decimal_places),
                                        decimal_places)
        ssim = f'{min_ssim} | {ssim_std} | {mean_ssim}'
        # Plot a line showing the variation of the SSIM throughout the
        # video.
    ...
    if args.calculate_psnr:
        psnr_scores = [psnr['metrics']['psnr'] for psnr
                       in file_contents['frames']]
        mean_psnr = force_decimal_places(round(np.mean(psnr_scores),
                                               decimal_places),
                                         decimal_places)
        min_psnr = force_decimal_places(round(min(psnr_scores),
                                              decimal_places),
                                        decimal_places)
        psnr_std = force_decimal_places(round(np.std(psnr_scores),
                                              decimal_places),
                                        decimal_places)   # Standard deviation.
